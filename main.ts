...
async function extractSearch(page: Page): Promise<SearchProduct[]> {
  const raw = await page.evaluate(`(() => {
    ...
    const getPrices = text => (String(text||'').match(/(?:Rs\\.?|NPR)\\s*[\\d,]+(?:\\.\\d+)?/gi)||[]).map(x=>Number(x.replace(/Rs\\.?|NPR/gi,'').replace(/,/g,'').trim())).filter(Number.isFinite);
    for (const link of document.querySelectorAll("a[href*='/products/']")) {
      const href = link.href; if (!href || !href.includes('/products/')) continue; let node = link;
      for (let d=0; d<8 && node; d++) {
        const text = clean(node.innerText || node.textContent), ps = getPrices(text);
        if (text.length > 20 && ps.length) {
          ...
          if (title && !seen.has(href)) { seen.add(href); const img=node.querySelector('img'); const dm=text.match(/(?:-\\s*)?(\\d+)%/); out.push({ title, url:href, price:ps[0], originalPrice:ps.length>1?Math.max(...ps):undefined, discount:dm?Number(dm[1]):undefined, image:img?.src||img?.getAttribute('data-src')||undefined }); }
          break;
        }
...
async function extractPdp(page: Page) {
  return await page.evaluate(`(() => {
    ...
    const specifications={}; const add=(k,v)=>{k=clean(k);v=clean(v);if(k&&v&&k.length<=150&&v.length<=1000)specifications[k]=v};
    ...
    for(const block of document.querySelectorAll("[class*='specification'],[class*='specs'],[class*='product-specification'],[class*='attribute']")) for(const row of block.querySelectorAll('li,div,p,tr')){const c=[...row.children].map(x=>clean(x.textContent)).filter(Boolean);if(c.length>=2){add(c[0],c.slice(1).join(' | '));continue;}const t=clean(row.textContent),i=t.indexOf(':');if(i>1)add(t.slice(0,i),t.slice(i+1));}
    for(const el of document.querySelectorAll('li,div,p')){const t=clean(el.textContent),i=t.indexOf(':');if(i>1&&t.length<500)add(t.slice(0,i),t.slice(i+1));}
    return { title:clean(document.querySelector('span.pdp-mod-product-badge-title')?.textContent||document.querySelector('h1')?.textContent), price:getPrice(document.querySelector('div.pdp-mod-product-price span.pdp-price')?.textContent), originalPrice:getPrice(document.querySelector('div.pdp-mod-product-price del')?.textContent), seller:clean(document.querySelector('div.seller-name__detail a')?.textContent||document.querySelector("[class*='seller'] a")?.textContent), image:document.querySelector('img.pdp-mod-common-image.gallery-preview-panel__image')?.src||document.querySelector('img')?.src, specifications };
  })()`);
}
...
async function detail(page: Page, p: SearchProduct, minPrice: number): Promise<Product|null> {
  try {
    ...
    return {...p,title:d.title||p.title,price:finalPrice,originalPrice:d.originalPrice||p.originalPrice,image:d.image||p.image,seller:d.seller||undefined,brand:specifications.Brand||specifications.brand||undefined,specifications};
...
async function saveToSupabase(product: Product, category: string, query: string): Promise<string> {
  const { data: row, error: productError } = await supabase.from('products').upsert({
    ...
    marketplace_id:DARAZ_MARKETPLACE_ID, external_id:product.itemId
  ...
  const { error: enrichmentError } = await supabase.from('product_enrichment_queue').upsert({
    product_id:productId, brand:product.brand||null,
    model:product.specifications.Model||product.specifications['Model Name']||null,
    product_type:category, parse_status:'needs_review', reason:'Daraz PDP collector',
    specifications:product.specifications, updated_at:new Date().toISOString()
  },{onConflict:'product_id'});
  if(enrichmentError) throw new Error(`Supabase enrichment upsert failed: ${enrichmentError.message}`);
  console.log(`SUPABASE SAVED | ${product.itemId} | NPR ${product.price}`);
  return productId;
}
